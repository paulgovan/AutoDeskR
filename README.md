...
We highly recommend that developers store their Client ID, Secret, and access tokens in a file called `.Renviron` and to access these keys with the `Sys.getenv()` function. This step will prevent authentication information from being stored in a publicly accessible location (e.g. GitHub repo). For more information on storing keys in the `.Renviron` file and accessing them with `Sys.getenv()`, see the appendix in this [API Best Practices](https://CRAN.R-project.org/package=httr/vignettes/api-packages.html) vignette.  
